...
| `valheim_handshakes_total`                            |                    | `Got handshake from client` occurrences: connection attempts before any password/ban/version check.                           |
| `valheim_connections_accepted_total`                  | `result`           | `Accepting connection k_EResult<result>` per result (`OK` = a join that passed the checks).                                    |
| `valheim_connection_state_changes_total`              | `state`            | Steam networking state transitions (`Connecting`, `Connected`, `ClosedByPeer`, `ProblemDetectedLocally`, `None`, …).          |
| `valheim_version_mismatches_total`                    |                    | Joins refused because the client's network version differed from the server's (`Network version check, their:N, mine:M`).     |
| `valheim_network_version`                             |                    | The server's network protocol version from that line.                                                                          |
| `valheim_loaded_objects`                              |                    | Unity objects loaded after the last asset unload (`Loaded Objects now: N`).                                                    |
| `valheim_asset_unloads_total`                         |                    | Unity `Unloading N unused Assets` passes.                                                                                      |
| `valheim_server_id`                                   | `id`               | The Steam game server id logged at start (value always `1`).                                                                   |

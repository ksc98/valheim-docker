...
- Mods come from Thunderstore by default. Set `MODS_REPOSITORY=hexium` to use [Hexium](https://hexium.gg), or prefix single entries with `hex:` / `ts:` to mix them. See [Mod Repositories](./tutorials/mod_repositories.md)
...
| TYPE                            | `Vanilla`                              | FALSE    | This can be set to `Vanilla`, `BepInEx`, or `ValheimPlus`                                                                                                                                                                                                                                                                                 |
...
| GALE_SYNC_CODE                  | `<nothing>`                            | FALSE    | A [Gale profile sync](./tutorials/gale_profile_sync.md) code. Installs the enabled mods of that profile at their exact versions, ahead of `MODS`. Needs `TYPE=BepInEx`.                                                                                                                                                                   |
| GALE_SYNC_CONFIGS               | `false`                                | FALSE    | Set to `true` to also copy the Gale profile's `BepInEx/config` files over the server's on every start.                                                                                                                                                                                                                                    |
| VALHEIM_PLUS_RELEASE            | `latest`                               | FALSE    | Which ValheimPlus release `TYPE=ValheimPlus` installs. Defaults to the newest one, pin it to a tag such as `0.10.1.2` to stay on a known version.                                                                                                                                                                                         |
| VALHEIM_PLUS_REPOSITORY         | `Grantapher/ValheimPlus`               | FALSE    | The GitHub repository `TYPE=ValheimPlus` pulls `ValheimPlus.dll` from. Change it to run a different fork.                                                                                                                                                                                                                                 |
| VALHEIM_PLUS_DOWNLOAD_URL       | `<derived>`                            | FALSE    | A full URL to a `ValheimPlus.dll`, used verbatim. Overrides both settings above.                                                                                                                                                                                                                                                          |
| MODS_REPOSITORY                 | `thunderstore`                         | FALSE    | Default repository for unprefixed `MODS` dependency strings: `thunderstore` (or `ts`) or `hexium` (or `hex`). Individual entries can override it with a `ts:` or `hex:` prefix. [Details](./tutorials/mod_repositories.md).                                                                                                               |
| HEXIUM_TOKEN                    | `<nothing>`                            | FALSE    | Optional Hexium API token (`hexium_...`). Sent as `Authorization: Bearer` to `hexium.gg` and its subdomains only. Not needed to install mods.                                                                                                                                                                                             |
| HEXIUM_BASE_URL                 | `https://hexium.gg`                    | FALSE    | Base URL used for Hexium API lookups. Override to point at a mirror, or at a local server for testing. Credentials are only attached to `hexium.gg` and its subdomains.                                                                                                                                                                   |
...
| THUNDERSTORE_USERNAME           | `<nothing>`                            | FALSE    | Legacy, non-functional. Thunderstore ignores HTTP Basic auth (it answers as an anonymous user rather than rejecting it), so this pair has no effect. Use `THUNDERSTORE_TOKEN` instead. Kept only for backwards compatibility.                                                                                                             |
| THUNDERSTORE_PASSWORD           | `<nothing>`                            | FALSE    | Legacy, non-functional. See `THUNDERSTORE_USERNAME`.                                                                                                                                                                                                                                                                                      |
| CONCURRENT_DOWNLOADS_ENABLED    | `true`                                 | FALSE    | Set to `false` to download mods one at a time (and skip chunked range downloads). Helps when Thunderstore/Cloudflare rate limits (HTTP 429) the burst of parallel requests.                                                                                                                                                               |
| MAX_CONCURRENT_DOWNLOADS        | `4`                                    | FALSE    | Total requests in flight at once, shared across mod downloads **and** the range chunks a single download splits into. Ignored when concurrency is disabled.                                                                                                                                                                               |
| DOWNLOAD_RETRY_ATTEMPTS         | `5`                                    | FALSE    | Attempts per mod request before giving up. Rate limited responses honor the `Retry-After` header and otherwise back off exponentially (capped at 60s).                                                                                                                                                                                    |
| DOWNLOAD_STAGGER_MS             | `250`                                  | FALSE    | Milliseconds between the start of each concurrent download, so requests are spaced instead of all opening at once. Set to `0` to disable.                                                                                                                                                                                                 |
| DOWNLOAD_POOL_MAX_IDLE_PER_HOST | `8`                                    | FALSE    | Idle keep-alive connections the HTTP pool retains per host for reuse.                                                                                                                                                                                                                                                                     |
| DOWNLOAD_POOL_IDLE_TIMEOUT_SECS | `90`                                   | FALSE    | How long an idle pooled connection is kept before being dropped.                                                                                                                                                                                                                                                                          |
| DOWNLOAD_REQUEST_TIMEOUT_SECS   | `300`                                  | FALSE    | Whole-request timeout for mod downloads. Raise for very large archives on slow links.                                                                                                                                                                                                                                                     |
...
| WEBHOOK_SUPPRESS_NOTIFICATIONS  | `0`                                    | FALSE    | Set to `1` to send Discord notifications silently (adds Discord's `SUPPRESS_NOTIFICATIONS` flag, `4096`). Messages still post to the channel but do not ping members. Discord webhooks only; see [webhooks.md](./webhooks.md).                                                                                                            |
| WEBHOOK_JOIN_URL                | `<nothing>`                            | FALSE    | An `https://` URL to place behind a **Join Server** button on successful start notifications. Blank disables the button. Discord rejects `steam://`, so point this at a redirect such as a reverse-proxied Huginn `/connect/remote`; see [webhooks.md](./webhooks.md).                                                                    |
...
| STEAMCMD_RESET_ON_FAILURE       | `1`                                    | FALSE    | Set to `0` to stop Odin clearing SteamCMD's download state and retrying once after a failed app update (the `state is 0x6 after update job` loop).                                                                                                                                                                                        |
...
| UPDATE_RESTART_CONTAINER        | `0`                                    | FALSE    | Set to `1` to stop the container after an update instead of relaunching the game inside it; the container's restart policy (or Kubernetes) starts it again on the new build. |
